...
const char* EMBEDDED_SCRIPT = R"RAW_SCRIPT(
...
static std::wstring g_tempScriptPath = L"";

void CleanupTempScript() {
    if (!g_tempScriptPath.empty()) {
        DeleteFileW(g_tempScriptPath.c_str());
    }
}
...
int main(int argc, char* argv[]) {
    ...
    g_tempScriptPath = std::wstring(tempPath) + L"dynamic_island_media_engine.ps1";
    
    HANDLE hFile = CreateFileW(
        g_tempScriptPath.c_str(),
        GENERIC_WRITE,
        FILE_SHARE_READ,
        NULL,
        CREATE_ALWAYS,
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
        DWORD written = 0;
        WriteFile(hFile, EMBEDDED_SCRIPT, (DWORD)strlen(EMBEDDED_SCRIPT), &written, NULL);
        CloseHandle(hFile);
    }
    
    std::wstring cmdLine = L"powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File \"" + g_tempScriptPath + L"\"";
    ...
    if (success) {
        ...
        std::cerr << " [X] Failed to launch backend engine. Error: " << GetLastError() << std::endl;
